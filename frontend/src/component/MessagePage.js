...
const MessagePage = () => {
  ...
  const handleUploadImage = async (e) => {
    try {
      ...
      if (!uploadPhoto?.url) throw new Error("Upload failed!");
  ...
  const handleSendMessage = (e) => {
    ...
    if (message.text || message.imageUrl || message.videoUrl) {
      if (socketConnection) {
        ...
        // Update local state to immediately show the sent message
                      ...
                      allMessage.map((msg,index)=>{
                        return(
                          <div className={` p-1 py-1 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md ${user._id === msg?.msgByUserId ? "ml-auto bg-teal-100" : "bg-white"}`}>
                            ...
                            <div  key={msg?._id || index} className='w-full relative'>
                              {msg?.imageUrl && (
                                  <img 
                                    src={msg?.imageUrl}
                                    alt="message-media"
